...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 载荷-位移关系 | 在线弹性小变形假设下应近似线性 | `load_magnitude` 从 `0` 到 `-10000 Pa`，`tip_disp_z` 从 `0` 线性变化到约 `-8.444 × 10⁻⁶ m` | 与线弹性预期一致 |
| 终值与静力结果一致性 | 瞬态末步应回到同一载荷下的静力解 | 瞬态末步约 `-8.444 × 10⁻⁶ m`，与粗网格静力结果一致 | 视频驱动结果与静力求解一致 |
...
| 热致变形 | 热膨胀应引起显著位移响应 | `tip_disp_z ≈ -5.628 × 10⁻⁴ m`（约 `-0.563 mm`） | 热-力耦合链路已打通 |
