# 红创科技多物理场仿真平台 - 七个现场 Demo 试验设计与结果对照
...
| `renders/cdp_damage.mp4` | **CDP 混凝土塑性损伤** | `inputs/cantilever_multiphysics_cdp.i` + `render_cdp.py` | `outputs/cantilever_multiphysics_cdp.e/.csv` + `renders/cdp_damage.mp4` | C30 混凝土标量损伤演化 (受拉/受压/总损伤) |
...
| 网格 | `outputs/cantilever_beam.msh`,现有输出文件统计为约 350 节点 / 985 四面体单元 |
...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 载荷-位移关系 | 在线弹性小变形假设下应近似线性 | `load_magnitude` 从 `0` 到 `-10000 Pa`,`tip_disp_z` 从 `0` 线性变化到约 `-8.444 × 10-6 m` | 与线弹性预期一致 |
| 终值与静力结果一致性 | 瞬态末步应回到同一载荷下的静力解 | 瞬态末步约 `-8.444 × 10-6 m`,与粗网格静力结果一致 | 视频驱动结果与静力求解一致 |
...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 钢区电位 | 由于电导率远高于混凝土,应近似等电位 | `potential_center_steel ≈ 1.000 V` | 钢区压降可忽略 |
| 界面电位 | 界面应接近钢侧电位,绝大部分电位降落在混凝土内 | `potential_interface ≈ 1.000 V` | 与高导电比预期一致 |
...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 温度场 | 梁内形成由冷端到热端的温度梯度 | `temp_mid ≈ 98.29 °C` | 温度变量已成功求解并输出 |
| 热致变形 | 热膨胀应引起显著位移响应 | `tip_disp_z ≈ -5.628 × 10-4 m`(约 `-0.563 mm`) | 热-力耦合链路已打通 |
| 损伤变量有界性 | `d` 应保持在 0~1 的演示区间内 | `damage_max ≈ 0.500` | 损伤变量稳定输出 |
...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 中心点复压力 | 中心点应呈现非零实部和虚部,反映幅值与相位 | `p_real_center ≈ 0.291 Pa`,`p_imag_center ≈ -0.420 Pa` | 复变量耦合成功 |
| 压力幅值 | 空腔内部响应幅值应小于 1 Pa 边界激励 | `|p| ≈ 0.511 Pa` | 量级合理 |
...
## Demo 7: `cdp_damage.mp4` — C30 混凝土塑性损伤 (CDP)

### 7.1 场景设计

该 demo 面向评标专家展示平台对**混凝土非线性材料模型**的完整支持能力：
1. 自定义 C++ 材料类 (`ConcreteDamagePlasticityStressUpdate`)；
2. 编译进 MOOSE 求解器；
3. 热-力-损伤三场耦合求解；
4. 损伤场输出与可视化。
...
| 网格 | `outputs/cantilever_beam.msh` |
| 材料 | C30 混凝土: E=30GPa, ν=0.20, f_t=2MPa, f_c=20MPa |
| 材料类 | `ConcreteDamagePlasticityStressUpdate` (自定义 C++) |
| 损伤模型 | 标量损伤: d_t (受拉), d_c (受压), d_total=max(d_t,d_c) |
| 热边界 | fixed_end=0°C, load_surface=100°C |
| 力学约束 | fixed_end: disp_x=disp_y=disp_z=0 |
| 耦合 | Temperature → Thermal Expansion → Stress → Damage → Stiffness Degradation |
| 求解 | Steady + PJFNK + hypre/boomeramg |
| 观测量 | tip_disp_z, temp_mid, damage_t_max, damage_c_max, damage_total_max |

### 7.3 输入与输出

- **C++ 源文件**:
  - `src/materials/ConcreteDamagePlasticityStressUpdate.h`
  - `src/materials/ConcreteDamagePlasticityStressUpdate.C`
- **输入**: `inputs/cantilever_multiphysics_cdp.i`
- **输出**:
  - `outputs/cantilever_multiphysics_cdp.e`
  - `outputs/cantilever_multiphysics_cdp.csv`
  - `renders/cdp_damage_contour.png`
  - `renders/cdp_damage.mp4`
- **渲染**: `render_cdp.py`
- **文档**: `CDP_BENCHMARK.md`
...
| 对比项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| 受拉损伤触发 | max_princ > f_t=2MPa 时 d_t > 0 | max_princ=8.06MPa, damage_t_max=0.988 | 受拉损伤成功演化 |
| 受压损伤未触发 | min_princ 绝对值 ≤ f_c=20MPa 时 d_c=0 | damage_c_max=0.000 | 符合悬臂梁热弯曲物理 |
| 损伤软化效应 | E_eff = E·(1-d), 损伤区刚度退化 | d_total=0.988 → E_eff≈0.36GPa (1.2%) | 显著软化 |
| 求解收敛 | 非线性迭代收敛 | 2 次迭代, nl_rel_tol=1e-8 | 稳定收敛 |
| C++ 源码交付 | MOOSE 材料类可编译 | .h/.C 源码 + CMakeLists + 注册代码 | 源码完整 |

### 7.5 专家说明

该 demo 验证了平台对**自定义非线性材料模型**的完整支持能力:
- C++ 层面: 继承 MOOSE ADComputeStressBase，实现 CDP 损伤本构；
- 编译层面: CMakeLists 配置, HongchuangApp 注册；
- 求解层面: 损伤变量参与应力计算, 刚度实时退化；
- 输出层面: damage_t/damage_c/damage_total 作为 MaterialProperty 输出；
- 可视化层面: 暗色品牌风格云图 + 时间演化动画。

与 Demo 4 (三场耦合) 的区别：Demo 4 使用演示型相场损伤变量，Demo 7 使用符合 GB 50010-2010 的 C30 混凝土真实 CDP 本构。
...
4. **接着讲 CDP 混凝土损伤 (`cdp_damage.mp4`)**: 展示自定义材料模型能力与非线性损伤演化;
5. **最后讲疲劳后处理**: 体现结果可进一步进入工程判据与寿命评估流程。
