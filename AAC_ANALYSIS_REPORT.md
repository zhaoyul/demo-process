...
> 生成日期: 2026-05-21 · 双倍软化塑性 + Pinching4 捏拢模型版
...
|------|------------|-------------|----------------|-------------|-------|----------|
| W-03 | 1113 | 24.00 | 73.9 | 71.0 | 13 | 170s | 双倍软化塑性 |
| W-04 | 1394 | 24.00 | 73.9 | 16.9 | 6 | 171s | 双倍软化塑性 |
| W-05 | 543 | 9.00 | 63.2 | 2.5 | 3 | 173s | 双倍软化塑性 |
| W-06 | 669 | 6.55 | 102.5 | 0.8 | 2 | 174s | 双倍软化塑性 |
| W-07 | 961 | 14.40 | 73.9 | 10.3 | 7 | 172s | 双倍软化塑性 |
| W-08 | 961 | 14.40 | 73.9 | 10.3 | 7 | 171s | 双倍软化塑性 |
| W-09 | 562 | 9.00 | 62.8 | 2.5 | 3 | 173s | 双倍软化塑性 |
...
MOOSE 无内置捏拢材料。采用 Lowes & Altoontash (2003) 的 **Pinching4 滞回模型**，
以 Python 独立实现，从 MOOSE 塑性结果提取骨架曲线作为输入。

Pinching4 的核心是三个捏拢参数：

| 参数 | 含义 | 值范围 | 捏拢效果 |
|------|------|--------|---------|
| **rDisp** | 再加载捏拢点位移比 | 0~1 (0=原点捏拢) | 越小→捏拢越深 |
| **rForce** | 再加载捏拢点力比 | 0~1 (0=零力捏拢) | 越小→原点力越低 |
| **uForce** | 卸载残余力比 | 0~1 | 越小→卸载越完全 |

### 7.2 捏拢参数对比

![Pinching4 对比](renders/aac_pinching4_comparison.png)

![捏拢细节](renders/aac_pinching4_detail.png)

### 7.3 与 MOOSE 各向同性塑性对比

| 特征 | MOOSE 塑性 | Pinching4 Mild | Pinching4 Moderate | Pinching4 Severe |
|------|-----------|---------------|--------------------|--------------------|
| rDisp | — | 0.5 | 0.35 | **0.2** |
| rForce | — | 0.3 | 0.15 | **0.05** |
| uForce | — | 0.05 | 0.01 | **0.0** |
| 滞回形状 | 饱满梭形 | 轻微捏拢 | 中等捏拢 | **强捏拢 (AAC-like)** |
| 捏拢程度 | 无 | ★★ | ★★★ | ★★★★★ |

### 7.4 结论

**严重捏拢参数 (rDisp=0.2, rForce=0.05)** 最接近加气砼墙体的反 S 形滞回特征：
- 原点附近刚度大幅降低（裂缝闭合/张开机制）
- 卸载残余力接近零
- 再加载初期刚度极低（裂缝尚未完全闭合）

**推荐采用 Pinching4 作为 AAC 墙体滞回分析的补充工具**，用 MOOSE 做承载力包络，
用 Pinching4 做捏拢滞回 → 耗能评估更安全（不捏拢的塑性模型高估耗能 ~2-3 倍）。
